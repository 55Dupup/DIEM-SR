...
class DegModel(nn.Module):
    ...
    def forward(self, inp,step=None):
        B, C, H, W = inp.shape
        h = H // self.scale
        w = W // self.scale
        ...
        x2=x
        # JPEG compression
        jpeger=DiffJPEG(differentiable=True).cuda()# simulate JPEG compression artifacts
        jpeg_range= [30, 95]
        jpeg_p = x.new_zeros(x.size(0)).uniform_(*jpeg_range)
        x = torch.clamp(x, 0, 1)  # clamp to [0, 1], otherwise JPEGer will result in unpleasant artifacts
        x = jpeger(x, quality=jpeg_p)
